...
void test_compact_meta_check_tensor() {
    ...
    status = get_model_path(classify_gender_model_path, MAX_STR_PATH_SIZE, "dima806/fairface_gender_image_detection",
                            "FP32");
    ck_assert(status == EXIT_STATUS_SUCCESS);
    status = get_model_path(classify_age_model_path, MAX_STR_PATH_SIZE, "dima806/facial_age_image_detection", "FP32");
    ck_assert(status == EXIT_STATUS_SUCCESS);
