...
!macro SilentComponentSelection
  ${If} ${Silent}
    ...
    ${IfNot} ${Errors}
      ...
      !insertmacro _SilentSetComponent 'python' 'c00_python'
      !insertmacro _SilentSetComponent 'env' 'c02_env'
      !insertmacro _SilentSetComponent 'samples' 'c03_samples'
      !insertmacro _SilentSetComponent 'development' 'c04_development'
    ${EndIf}
  ${EndIf}
!macroend
...
!macro CheckPythonInstallation
  ; Check user installation
  ReadRegStr $R1 HKCU 'SOFTWARE\Python\PythonCore\${PYTHON_SHORT_VERSION}' 'Version'
  ${If} $R1 != ''
    MessageBox MB_OK|MB_ICONINFORMATION 'Python $R1 is already installed for the current user. Installer will not install system-wide Python.' /SD IDYES
    !insertmacro UnselectSection ${c00_python}
  ${EndIf}
 
  ; Check system-wide installation
  ReadRegStr $R1 HKLM 'SOFTWARE\Python\PythonCore\${PYTHON_SHORT_VERSION}' 'Version'
  ${If} $R1 != ''
    ${VersionCompare} '$R1' '${PYTHON_VERSION}' $R4
    ${If} $R4 != 2 ; not older than bundled
      MessageBox MB_OK|MB_ICONINFORMATION 'Python $R1 is already installed. Installer will not install system-wide Python.' /SD IDYES
      !insertmacro UnselectSection ${c00_python}
    ${EndIf}
  ${EndIf}
!macroend

!macro CheckDependenciesFunc
  Function CheckDependencies
    Call CheckGStreamerInstallation

    !insertmacro SectionFlagIsSet ${c00_python} ${SF_SELECTED} DoCheckPython SkipCheckPython
    DoCheckPython:
    !insertmacro CheckPythonInstallation
    SkipCheckPython:
  FunctionEnd
!macroend

!define CUSTOMFUNCTION_COMPONENTS_LEAVE CheckDependencies
!define DLSTREAMER_DEPENDENCIES_CHECK '!insertmacro CheckDependenciesFunc'
...
Function ${un}UninstallGStreamerInno
  ...
  ; TODO remove. 1.28.1 uninstaller does not remove GStreamer registry entries
  DeleteRegKey HKLM 'SOFTWARE\GStreamer1.0\x86_64'
FunctionEnd
!macroend
...
!macro InstallGStreamer
  ...
  ; TODO use type=runtime in 1.28.2
  DetailPrint 'Execute GStreamer ${GSTREAMER_VERSION} installer'
  nsExec::ExecToLog '$INSTDIR\deps\gstreamer-1.0-msvc-x86_64-${GSTREAMER_VERSION}.exe /VERYSILENT /LOG /TYPE=full /ALLUSERS'
  Pop $R3
  ${If} $R3 != 0
...
!macro InstallPython
  ...
  ReadRegStr $R1 HKCU 'SOFTWARE\Python\PythonCore\${PYTHON_SHORT_VERSION}' 'Version'
  ${If} $R1 != ''
    DetailPrint 'Found Python $R1 installed for current user, skipping system-wide installation'
    Goto SkipPython
  ${EndIf}

  ; Check system-wide installation
  ReadRegStr $R1 HKLM 'SOFTWARE\Python\PythonCore\${PYTHON_SHORT_VERSION}' 'Version'
  DetailPrint 'Installed Python version: $R1'
  ${IfThen} $R1 == '' ${|} Goto InvokePythonInstaller ${|}

  ; Result: 0=equal, 1=installed newer, 2=bundled newer
  ${VersionCompare} '$R1' '${PYTHON_VERSION}' $R4
  ${If} $R4 = 2
    Goto InvokePythonInstaller
  ${Else}
    DetailPrint 'Python version $R1 already installed, skipping'
    Goto SkipPython
  ${EndIf}

  InvokePythonInstaller:
  SetOutPath '$INSTDIR\deps'
  File '${INSTALLER_DEPS_DIR}\python-${PYTHON_VERSION}-amd64.exe'

  Push '${PYTHON_INSTALLER_HASH}'
  Push '$INSTDIR\deps\python-${PYTHON_VERSION}-amd64.exe'
  Call VerifyFileHash
  Pop $R5
  ${If} $R5 == 'FAIL'
    Goto SkipPython
  ...
  DetailPrint 'Execute Python ${PYTHON_VERSION} installer'
  nsExec::ExecToLog '$INSTDIR\deps\python-${PYTHON_VERSION}-amd64.exe /quiet InstallAllUsers=1 PrependPath=1'
  Pop $R3
  ${If} $R3 != 0
    DetailPrint 'Python installation failed. Error code: $R3'
    MessageBox MB_OK|MB_ICONEXCLAMATION 'Failed to install Python.' /SD IDYES
  ${EndIf}
...
!macro InstallExtras
  ...
  !insertmacro SectionFlagIsSet ${c00_python} ${SF_SELECTED} DoInstallPython SkipInstallPython
  DoInstallPython:
  !insertmacro InstallPython
  SkipInstallPython:

  SetCompress auto

  !insertmacro SectionFlagIsSet ${c02_env} ${SF_SELECTED} DoSetupEnv SkipSetupEnv
  DoSetupEnv:
