...
| `buffer.map(READ\|WRITE)` fails in `do_transform_ip` | Custom element cannot modify pixel data — returns `success=False` | Override `do_prepare_output_buffer` to allocate a new buffer — see [Pattern 7: Buffer Mutability](./design-patterns.md#buffer-mutability) |
