...
To detect GPUs on the platform and map them to their corresponding
`/dev/dri/render*` devices, use the following command:

```bash
for d in /dev/dri/render*; do
	vendor=$(cat "/sys/class/drm/$(basename "$d")/device/vendor" 2>/dev/null)
	pci=$(basename "$(readlink "/sys/class/drm/$(basename "$d")/device" 2>/dev/null)")
	name=$(lspci -s "$pci" 2>/dev/null | sed -E 's/^[^:]+: //')
	printf '%s -> vendor=%s, pci=%s, name=%s\n' "$d" "$vendor" "$pci" "$name"
done
```
