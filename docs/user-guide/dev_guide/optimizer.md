...
Currently the DL Streamer Optimizer focuses mainly on DL Streamer elements, specifically the `gvadetect` and `gvaclassify`. The produced pipeline could still have potential for further optimization by transforming other elements.

Multi-stream pipelines (those utilizing the `tee` element) are also currently not supported.

## Prerequisites

Before using the DL Streamer Optimizer, ensure you have the OpenVINO Python module installed.
...
While the DL Streamer Optimizer is running, you can pause and resume the search at any time by pressing the **Spacebar** in the terminal where the tool is running.
...
>**Note**\
>The pause key is only active while the DL Streamer Optimizer is running as a CLI tool in terminal with keyboard input available. It is not available in non-interactive or piped environments.
