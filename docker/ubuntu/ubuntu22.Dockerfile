...
RUN apt-get update && apt-get install --no-install-recommends -y gnupg=\* && \
    ...
    rm -rf /var/lib/apt/lists/*
RUN \
    echo "deb https://apt.repos.intel.com/openvino/2026 ubuntu22 main" | tee /etc/apt/sources.list.d/intel-openvino-2026.list && \
...
RUN curl -fsSL https://apt.repos.intel.com/intel-gpg-keys/GPG-PUB-KEY-INTEL-SW-PRODUCTS.PUB | \
    ...
    echo "deb [signed-by=/usr/share/keyrings/intel-sw-products.gpg] https://apt.repos.intel.com/openvino/2026 ubuntu22 main" \
