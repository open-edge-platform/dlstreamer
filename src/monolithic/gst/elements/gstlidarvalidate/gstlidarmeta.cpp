...
GType lidar_meta_api_get_type(void) {
    ...
    if (g_once_init_enter(&type)) {
        ...
        GType _type = existing ? existing : gst_meta_api_type_register("LidarMetaAPI", tags);
...
LidarMeta *add_lidar_meta(GstBuffer *buffer, guint lidar_point_count, const std::vector<float> &lidar_data, size_t frame_id, GstClockTime exit_lidarparse_timestamp, guint stream_id) {
    ...
    GST_DEBUG("Adding LidarMeta to buffer with lidar_point_count=%u frame_id=%zu stream_id=%u exit_ts=%" GST_TIME_FORMAT,
    ...
    GST_DEBUG("LidarMeta added successfully: lidar_point_count=%u, lidar_data_size=%zu, frame_id=%zu, stream_id=%u, exit_ts=%" GST_TIME_FORMAT,
              meta->lidar_point_count, meta->lidar_data.size(), meta->frame_id, meta->stream_id, GST_TIME_ARGS(meta->exit_lidarparse_timestamp));
