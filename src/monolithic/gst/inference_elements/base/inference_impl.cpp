...
// Detects the MonoDETR-style auxiliary inputs (a 3x4 camera-projection "calib" input and a [B,2]
// "img_sizes" input) from the model file and, when present, appends ModelInputProcessorInfo entries
// so the backend feeds them. Values come from the gvamono3d "calibration-file" property (falling
// back to the source frame size). Only runs for the gvamono3d element.
void AppendMono3DAuxInputs(GvaBaseInference *gva_base_inference, const std::string &model_file,
    ...
    if (gva_base_inference->type != GST_GVA_MONO3D_TYPE)
        return;
    ...
    std::string calib_layer, img_sizes_layer, image_layer;
    for (const auto &kv : input_shapes) {
        ...
        if (dims.size() == 3 && dims[1] == 3 && dims[2] == 4)
            ...
            image_layer = kv.first; // NCHW/NHWC image input
    }
    ...
    for (const auto &p : input_processor_info)
        if (p->layer_name == calib_layer || p->layer_name == img_sizes_layer || p->layer_name == image_layer)
            return;
    ...
    GstGvaMono3d *mono3d = reinterpret_cast<GstGvaMono3d *>(gva_base_inference);
    ...
    // Multi-input models require every input declared; mark the image input explicitly so the
    // backend applies standard image pre-processing (resize/CSC/normalize) to it.
    if (!image_layer.empty()) {
        auto image_info = std::make_shared<ModelInputProcessorInfo>();
        image_info->format = "image";
        image_info->layer_name = image_layer;
        image_info->precision = "U8";
        image_info->params = gst_structure_new_empty("image");
        input_processor_info.push_back(image_info);
    }
...
InferenceImpl::Model InferenceImpl::CreateModel(GvaBaseInference *gva_base_inference, const std::string &model_file,
    ...
    // MonoDETR's backbone overflows f16 on GPU under the default PERFORMANCE mode (NaN outputs);
    // default the execution mode to ACCURACY unless the user set it explicitly via ie-config.
    if (gva_base_inference->type == GST_GVA_MONO3D_TYPE && gva_base_inference->device &&
        std::string(gva_base_inference->device).find("GPU") != std::string::npos) {
        auto &inference_cfg = ie_config[KEY_INFERENCE];
        if (inference_cfg.find("EXECUTION_MODE_HINT") == inference_cfg.end())
            inference_cfg["EXECUTION_MODE_HINT"] = "ACCURACY";
    }
