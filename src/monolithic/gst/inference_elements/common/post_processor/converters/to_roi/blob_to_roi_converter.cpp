...
BlobToMetaConverter::Ptr BlobToROIConverter::create(BlobToMetaConverter::Initializer initializer,
    ...
    if (!custom_postproc_lib.empty())
        ...
        return BlobToMetaConverter::Ptr(new BoxesLabelsConverter(std::move(initializer), confidence_threshold, iou_threshold));
    else if (converter_name == BoxesScoresConverter::getName())
        return BlobToMetaConverter::Ptr(new BoxesScoresConverter(std::move(initializer), confidence_threshold, iou_threshold));
    else if (converter_name == BoxesConverter::getName())
        return BlobToMetaConverter::Ptr(new BoxesConverter(std::move(initializer), confidence_threshold, iou_threshold));
