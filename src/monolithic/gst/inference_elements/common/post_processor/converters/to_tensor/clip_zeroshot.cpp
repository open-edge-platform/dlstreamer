...
ClipZeroShotConverter::ClipZeroShotConverter(BlobToMetaConverter::Initializer initializer)
    : BlobToTensorConverter(std::move(initializer)) {
    topk_ = std::max<uint32_t>(1, getZeroshotTopk());
    embeddings_ = getZeroshotEmbeddings();

    if (embeddings_.empty()) {
        throw std::invalid_argument("Zero-shot class embeddings are not set. Use gvaclassify "
                                    "zeroshot-embeddings-file to supply the class bank.");
    }

    const auto &labels = getLabels();
    if (!labels.empty() && labels.size() != embeddings_.num_classes) {
        throw std::invalid_argument("Number of labels (" + std::to_string(labels.size()) +
                                    ") does not match number of embeddings (" +
                                    std::to_string(embeddings_.num_classes) + ").");
    }

    logit_scale_ = embeddings_.logit_scale;
    ...
    GVA_INFO("CLIP zero-shot: %zu classes, embedding_dim=%zu, top-k=%u, logit_scale=%.3f, unknown_threshold=%s.",
             embeddings_.num_classes, embeddings_.embedding_dim, topk_, static_cast<double>(logit_scale_),
             embeddings_.unknown_threshold >= 0.0 ? std::to_string(embeddings_.unknown_threshold).c_str() : "disabled");
}

std::vector<float> ClipZeroShotConverter::computeSimilarities(const float *image_embedding,
                                                              std::size_t image_embedding_size) const {
    const std::size_t embedding_dim = embeddings_.embedding_dim;
    if (image_embedding_size < embedding_dim) {
        throw std::runtime_error("Image embedding dimension " + std::to_string(image_embedding_size) +
                                 " is smaller than label embedding dimension " + std::to_string(embedding_dim) + ".");
    }
    if (image_embedding_size > embedding_dim) {
        throw std::runtime_error("Image embedding dimension " + std::to_string(image_embedding_size) +
                                 " does not match label embedding dimension " + std::to_string(embedding_dim) +
                                 ". Re-export the vision model and embeddings with the same CLIP model.");
    }

    const auto normalized_image = normalizeVector(image_embedding, embedding_dim);
    std::vector<float> similarities(embeddings_.num_classes, 0.0F);

    for (std::size_t row = 0; row < embeddings_.num_classes; ++row) {
        const float *class_embedding = embeddings_.class_embeddings.data() + row * embedding_dim;
        float dot = 0.0F;
...
std::vector<float> ClipZeroShotConverter::computeProbabilities(const std::vector<float> &similarities) const {
...
TensorsTable ClipZeroShotConverter::convert(const OutputBlobs &output_blobs) {
    ...
    const auto &labels = getLabels();
    const uint32_t actual_topk = static_cast<uint32_t>(std::min<size_t>(topk_, embeddings_.num_classes));
    ...
    for (size_t frame_index = 0; frame_index < batch_size; ++frame_index) {
        ...
        std::vector<size_t> indices(embeddings_.num_classes);
        ...
        const bool is_unknown = (embeddings_.unknown_threshold >= 0.0) &&
                                (static_cast<double>(similarities[top_class]) < embeddings_.unknown_threshold);
        ...
        for (uint32_t rank = 0; rank < emitted; ++rank) {
            ...
            gst_structure_set(classification_result.gst_structure(), "tensor_id", G_TYPE_INT,
                              ...
                              "zs_mode", G_TYPE_BOOLEAN, TRUE, "zs_unknown", G_TYPE_BOOLEAN, is_unknown ? TRUE : FALSE,
                              "zs_model", G_TYPE_STRING, model_name.c_str(), NULL);
